...
class BooksAdapter extends CursorAdapter implements FilterQueryProvider {
  private static final String[] PROJECTION_IDS_AND_TITLE = new String[] {
      BooksStore.Book._ID, BooksStore.Book.INTERNAL_ID, BooksStore.Book.TITLE, BooksStore.Book.SORT_TITLE };
  ...
  BooksAdapter(ShelvesActivity activity) {
    super(activity, activity.managedQuery(BooksStore.Book.CONTENT_URI, PROJECTION_IDS_AND_TITLE, null, null,
        BooksStore.Book.DEFAULT_SORT_ORDER), true);
    ...
    mDefaultCoverBitmap = BitmapFactory.decodeResource(activity.getResources(), R.drawable.unknown_cover);
  ...
  public void bindView(View view, Context context, Cursor c) {
    ...
    if (activity.getScrollState() == AbsListView.OnScrollListener.SCROLL_STATE_FLING || activity.isPendingCoversUpdate()) {
      holder.title.setCompoundDrawablesWithIntrinsicBounds(null, null, null, mDefaultCover);
      holder.queryCover = true;
    } else {
      holder.title.setCompoundDrawablesWithIntrinsicBounds(null, null, null, ImageUtilities.getCachedCover(bookId, mDefaultCover));
  ...
  public Cursor runQuery(CharSequence constraint) {
    if (constraint == null || constraint.length() == 0) {
      return mActivity.managedQuery(BooksStore.Book.CONTENT_URI, PROJECTION_IDS_AND_TITLE, null, null, BooksStore.Book.DEFAULT_SORT_ORDER);
    }
    ...
    return mActivity.managedQuery(BooksStore.Book.CONTENT_URI, PROJECTION_IDS_AND_TITLE, mSelection, arguments2,
